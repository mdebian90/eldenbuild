...
(async function () {
  ...
  try {
    ...
    if (r.status !== 200) {
      ...
      GridExplorar.innerHTML = "<div style='padding:16px'>No se pudieron cargar las builds ("
        + r.status + ").</div>";
      return;
    }
    ...
    GridExplorar.innerHTML = "<div style='padding:16px'>Error de red al cargar builds.</div>";
    return;
  }

  if (datos.length === 0) {
    GridExplorar.innerHTML = "<div style='padding:16px'>No hay builds públicas por ahora.</div>";
    return;
  }
  ...
  Buscar.addEventListener("input", function () {
    ...
    for (let indice = 0; indice < tarjetas.length; indice++) {
      ...
      const nombre = tarjetas[indice].querySelector(".NombreBuild").textContent.toLowerCase();
